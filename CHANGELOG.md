...
- 3 yeni yerli skill (`skills/`): mevcut/legacy projeyi devralırken çalışan analiz üçlüsü.
  ...
  - `analyze-functional`: Ürün haritası — ekran haritası, buton→endpoint→tablo aksiyon zincirleri, kullanıcı akışları, domain varlıkları + glossary adayları, "ne yapıyor / ne yapmıyor" özeti (yarım feature tespiti dahil). Kod gerçeğin kaynağı; kanıtsız iddia `(belirsiz)` işaretlenir. §2'yi yazar; `grill-with-docs` Rol 1'in girdisini üretir. Örnek: `examples/01-erp-order-flow.md`.
  - `analyze-project`: **Orchestrator** — `analyze-codebase` → `analyze-functional` zincirini otomatik çalıştırır ("öner, kullanıcı tetiklesin" konvansiyonunun `dispatch-agents` gibi bilinçli istisnası; zincir raporla biter, sonraki skill'ler yalnızca önerilir). Tazelik kontrolü (`analyzed_commit` vs HEAD; tazeleme modunda "son analizden bu yana" diff özeti), sentez (§1 Yönetici Özeti, §5 Memory Besleme, §6 Sıradaki Adımlar), yerleşim (`docs/memory/99-meta/` + MOC linki; memory yoksa kök). Cursor için Manuel Mod. Örnek: `examples/01-full-analysis-report.md` (birleşik rapor + tazeleme modu).
- Memory sınırı net: analiz **makine gözlemi** üretir, tek yazdığı dosya `PROJECT_ANALYSIS.md` — `20-modules/`, `30-decisions/`, `50-glossary/`, `CONTEXT.md`'ye yalnızca insan-onaylı skill'ler (memorize-module, scaffold-adr, grill-with-docs) yazar; rapor onlara aday listesi besler (§5).
- 3 yeni eval JSON (`analyze-codebase`, `analyze-functional`, `analyze-project` — her biri 10 pos + 10 neg; `analyze-project` pozitifi "Tüm projeyi analiz et", memorize-module negatifiyle tutarlı).

### Changed (Hafta 12 — Analiz Wiring)
- `skills/memorize-module/SKILL.md`: Süreç'e "0. Hızlandırıcı — PROJECT_ANALYSIS.md (varsa)" adımı — modül seçimi (§5.1 churn top-5), kapsam/dosya listesi (§3), soruları rapor bulgularına keskinleştirme, İlişkiler/Kararlar taslak ön-doldurma. İki koruma: bayatlık kontrolü (`analyzed_commit` vs HEAD) + güncel koddan doğrulama adımı hiçbir durumda atlanmaz; rapor opsiyonel hızlandırıcıdır, önkoşul değil.
- `skills/setup-rubion-skills/SKILL.md`: 5 Legacy matris satırına `analyze-project` eklendi (`setup-memory`'den hemen sonra — rapor `99-meta/`'ya doğsun); Faz C'ye "Analiz ön-adımı (legacy)" kuralı (`setup-memory → analyze-project → grill-with-docs` zinciri); A.7'ye "Legacy + hiç döküman yok → analiz dökümanını analyze-project üretir" maddesi (`PROJECT_ANALYSIS.md` analiz dökümanı sayılır); Faz D tespit matrisine `PROJECT_ANALYSIS.md` varlık/tazelik + kökten `99-meta/`'ya taşıma satırları; Legacy × .NET örnek yol haritası 9 adıma güncellendi.
- Doküman: `docs/skills-catalog.md` (29→32, yeni 🔬 Analyze kategorisi + "mevcut projeyi anlama" chain'i + karar ağacına "Mevcut projeyi tanıyacağım" dalı), `docs/getting-started.md` (Senaryo 2'ye 0b analyze-project aşaması + checklist + karşılaştırma tablosu), `docs/skills-aciklama.md` (29→32 + 🔬 Analyze bölümü), `README.md` (skill sayısı).
...
- **grill-with-docs wizard'a bağlandı:** `skills/setup-rubion-skills/SKILL.md` Faz A'ya A.7 (Soru 5 — Domain/Analiz Dökümanı tespiti) eklendi; serbest `.md` analiz dökümanı var ama `CONTEXT.md` yoksa `grill-with-docs` path'in ilk build adımı olarak önerilir (scaffold'dan önce — domain glossary + monolith/mikroservis kararı netleşsin). Faz C matrisine "grill ön-adımı" kuralı, Faz D re-entry tespitine `CONTEXT.md`/`CONTEXT-MAP.md` satırı. Daha önce wizard grill'i hiç önermiyordu (getting-started'da vardı ama wizard'da yoktu).
- **grill-with-docs'un iki rolü netleştirildi + doküman tutarsızlığı kapatıldı:** Skill'e `<iki-kullanım>` bloğu eklendi — **Rol 1** (init'te ham analiz `.md` → CONTEXT.md + ADR) ve **Rol 2** (her feature'da PRD denetimi, `to-prd → grill → to-issues` arası). `docs/getting-started.md`: feature döngüsü grill'i atlıyordu (`to-prd → to-issues`); her iki senaryoya **8b grill denetim adımı** + başa "iki rol" notu eklendi, init adımları "Rol 1" etiketlendi. `docs/skills-catalog.md`: grill girişi iki rolü anlatacak şekilde + chain notu. `ADAPTATION.md` güncellendi. (Önceden katalog grill'i feature döngüsüne koyuyordu, getting-started koymuyordu — çelişki giderildi.)
- **Mono-repo kök düzeni:** `docs/stack-conventions.md`'ye "Mono-repo Kök Düzeni" bölümü — full-stack repo'da kök `src/` altında `backend/` + `frontend/` ayrımı (monolith + mikroservis örnekleri, çift-src yasağı, backend-only/frontend-only istisnaları). Her iki scaffold skill'ine "Mono-repo Yerleşimi" notu: frontend aynı repo'da ise backend `src/backend/`, frontend `src/frontend/` altına; kullanıcıya "frontend de bu repo'da mı" sorulur.
